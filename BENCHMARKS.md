...
| **Machine B** | Mobile Workstation | Windows 11 Pro | **NVIDIA GeForce RTX 4060 Laptop 8GB** (Ada `sm_89`)<br>+ **AMD Radeon 890M 16 CUs** (`gfx1150`)<br>+ **AMD Ryzen AI 9 HX 370** (12C/24T Zen 5) | **32 GB LPDDR5X Total**<br>• 8 GB GDDR6 Dedicated (dGPU)<br>• 15.5 GB Dynamic UMA (iGPU DWM) | **256 GB/s** (dGPU)<br>~120 GB/s (UMA) | **Bare-Metal SASS (`nvcuda.dll`)**<br>+ **Direct3D 12 Compute (HLSL Wave32)**<br>+ **AVX-512 SIMD Host CPU** |
| **Machine C** | Workstation Mini PC | Linux x64 (Fedora) | **AMD Radeon 890M 16 CUs** (RDNA 3.5 `gfx1150`)<br>+ **AMD Ryzen AI 9 HX 370** (12C/24T Zen 5)<br>+ **XDNA 2 NPU** (50 TOPS `aie2p`) | **64 GB LPDDR5X-7500 Unified**<br>• **47 GB usable unified VRAM**<br>• Direct `/dev/kfd` HSA access | **~120 GB/s** (Unified) | **Bare-Metal HIP / ROCm (`libamdhip64.so`)**<br>+ **Vulkan Cooperative Matrix (`libvulkan.so.1`)** |
...
* **Primary Engines**: Pure C# Bare-Metal SASS (`nvcuda.dll`) on dGPU; Direct3D 12 Compute (`HLSL Wave32`) on iGPU.

| Accelerator & Engine | Model | Quantization | Footprint | Active Params | Prompt Prefill | Generation Rate | Turnaround |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **RTX 4060 (Bare-Metal SASS)** | **Qwen3-4B-Instruct** | `Q4_K_M` | 2.33 GB | 4.0 B | **177.90 tok/s** | 🚀 **65.99 tok/s** | **0.42 s** |
| **RTX 4060 (Bare-Metal SASS)** | **Qwen2.5-7B-Instruct** | `Q4_K_M` | 4.68 GB | 7.0 B | **104.20 tok/s** | **41.92 tok/s** | **0.78 s** |
| **RTX 4060 (Bare-Metal SASS)** | **DeepSeek-R1-Distill-Qwen-7B** | `Q4_K_M` | 4.36 GB | 7.0 B | **102.50 tok/s** | **42.40 tok/s** | **0.82 s** |
| **RTX 4060 (Bare-Metal SASS)** | **Meta LLaMA 3.1 8B Instruct** | `Q4_K_M` | 4.58 GB | 8.0 B | **91.80 tok/s** | **44.05 tok/s** | **0.58 s** |
| **RTX 4060 (Bare-Metal SASS)** | **Qwen2.5-Coder-7B Enterprise** | `Q8_0` | 7.54 GB | 7.0 B | **82.30 tok/s** | **28.43 tok/s** | **2.00 s** |
...
* **Engines Compared**: **Bare-Metal AMD HIP / ROCm (`/dev/kfd`)** vs. **Universal Vulkan Cooperative Matrix (`VK_KHR_cooperative_matrix`)**.
...
| :--- | :--- | :---: | :---: | :--- | :---: | :---: | :--- |
| **Qwen2.5-Coder-7B** | Dense | `Q4_K_M` | 4.36 GiB | **Vulkan Cooperative Matrix** | 🚀 **369.34 tok/s** | 🚀 **20.04 tok/s** | Highest generation speed on 890M |
| **Qwen2.5-Coder-7B** | Dense | `Q4_K_M` | 4.70 GiB | **Bare-Metal HIP / ROCm** | **253.60 tok/s** | **17.66 tok/s** | Direct `/dev/kfd` AQL doorbell dispatch |
| **Gemma-4-26B-A4B-it** | **MoE (128 Experts, Top-8)** | `Q4_K_M` | **15.63 GiB** | **Vulkan Cooperative Matrix** | 🚀 **285.41 tok/s** | 🚀 **29.00 tok/s** | **5.8x faster than dense 27B** |
| **Gemma-4-26B-A4B-it** | **MoE (128 Experts, Top-8)** | `Q4_K_M` | **15.63 GiB** | **Bare-Metal HIP / ROCm** | **103.16 tok/s** | **12.12 tok/s** | 32K context buffer allocated (19 GB) |
| **Qwen3.6-27B-UD** | Dense | `Q4_K_XL` | **16.39 GiB** | **Vulkan Cooperative Matrix** | **94.43 tok/s** | 🐌 **4.97 tok/s** | Memory bus bandwidth saturated |
| **Qwen3.6-27B-UD** | Dense | `Q4_K_XL` | **16.39 GiB** | **Bare-Metal HIP / ROCm** | **23.36 tok/s** | 🐌 **2.19 tok/s** | 32K context buffer allocated (23 GB) |
| **Qwen-Coder-Latest** | Dense (Unquantized) | `F16` | **14.00 GiB** | **Bare-Metal HIP / ROCm** | **36.00 tok/s** | **2.71 tok/s** | Full 16-bit unquantized float weights |
...
| :--- | :---: | :---: | :---: | :--- | :--- | :---: | :---: |
| **Gemma-4-26B-A4B-it** | 25.2 B / **4.0 B** (128 Experts) | `Q4_K_M` | 15.63 GiB | **Machine C (Radeon 890M 64GB)** | Vulkan Cooperative Matrix | 🚀 **285.41 tok/s** | 🚀 **29.00 tok/s** |
...
| Model Architecture & Quant | Vulkan Cooperative Matrix (`VK_KHR_coopmat`) | Bare-Metal HIP / ROCm (`/dev/kfd`) | Direct3D 12 Compute (HLSL Wave32) |
| :--- | :---: | :---: | :---: |
